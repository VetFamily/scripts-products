...
def insert_source_code(df, source_id):
    ...
    if source_id != constant.SOURCE_DIRECT_ID:
        ...
        logging.debug(f"Table centrale_produit : {len(df_temp_new_cp)} elements created")
        ...
        df_source_tmp = \
            pd.merge(df_source_tmp, df_tmp,
                     on=['product_code', 'cirrina_pricing_condition_id', 'laboratoire_id'], how='left')
        df_source_tmp = df_source_tmp[df_source_tmp['centrale_produit_id'].isnull()]
        del df_tmp
    ...
    if len(df_source_tmp) > 0:
        ...
        df_temp_new_cpd.to_sql('centrale_produit_denominations', engine, if_exists='append', index=False,
                                chunksize=1000)
        logging.debug(f"Table centrale_produit_denominations : {len(df_temp_new_cpd)} elements created")
...
def process_products():
    ...
    products_out_dir = os.path.join(products_dir, constant.DIR_ARCHIVES, now + "_purchases")
    ...
    df_final = pd.DataFrame(columns=[
        'Id', 'Dénomination_temp', 'Conditionnement_temp', 'Laboratoire_temp',
    ...
    if types_especes.duplicated(subset=['source_id', 'supplier_id', 'type_source']).any():
        raise ValueError("Types_Especes.xlsx contains duplicated rows: "
    ...
    for source_id, df_group in df.groupby(['source_id']):
        ...
        if source_id in [constant.SOURCE_DIRECT_ID, constant.SOURCE_KRUUSE_ID]:
            ...
            df_source = pd.concat([df_source, df_source2], axis=0, sort=False, ignore_index=True)
        ...
        df_temp_not_matched = df_temp[~df_temp['temp_id'].isin(df_source['temp_id'])].copy()
        df_source = pd.concat([df_source, df_temp_not_matched], axis=0, sort=False, ignore_index=True)
        ...
        df_source_copy['laboratoire_id'] = pd.to_numeric(
            df_source_copy['laboratoire_id'],
            errors='coerce'
        )

        types_especes_restricted = types_especes[types_especes['source_id'] == source_id].copy()
        types_especes_restricted.rename(
            columns={'type': 'new_type', 'especes': 'new_especes'},
            inplace=True
        )
    ...
    writer = pd.ExcelWriter(
        os.path.join(products_out_dir, now + "_" + country_name + "_new_products_purchases.xlsx"),
        engine='xlsxwriter')
    ...
    excel_cols = (alphabet + ['A' + x for x in alphabet] +
    ...
    shutil.copy(
        os.path.join(products_out_dir, now + "_" + country_name + "_new_products_purchases.xlsx"),
        os.path.join(products_out_dir, now + "_" + country_name + "_new_products_purchases_source.xlsx"))
...
def process_suppliers():
    ...
    suppliers_out_dir = os.path.join(suppliers_dir, constant.DIR_ARCHIVES, now + "_purchases")
    ...
    if file_exist:
        ...
        df.rename(columns={'source_id': 'centrale_id',
                           'supplier': 'nom_laboratoire'}, inplace=True)
...
if __name__ == "__main__":
    ...
    if debug:
        ...
        logging.basicConfig(
            filename=os.path.join(params_logging["url"], constant.LOG_PURCHASES_FILENAME + constant.LOG_EXTENSION),
